...
`main` is at `72f877f2d247cf518d00845e78b94bf461ad02e2` (PR #3 foundation merge). The current final-closure branch is `feat/graphql-phase4-final-closure`, synchronized with origin at `dd097d1dcbd8edddf68d4e4ed43b95198c410f8c` (`dd097d1 feat: connect GraphQL history replay workflow`).

The branch preserves saved-request CRUD (`9480968`), draft/tab workflow (`5163345`), app-shell wiring (`b16b134`), centralized HTTP tab execution (`5b8535b`), HTTP foundations (`2b195c1`, `f6e7d36`), subscription workflow (`927683b`), schema workflows (`503c2a8`), and history UI (`dd097d1`). Final full-suite/build evidence must be refreshed after the latest commits. This is not Phase 4 closure evidence: there is no final PR, final CI run, merge to `main`, repository-closure document, or milestone tag.
...
| HTTP history and comparison | PARTIAL / MISSING | Sanitized searchable history, retention, delete, replay, and a connected UI now exist. Pin/tags/notes, full filters, export/diagnostic bundle, immutable failure snapshots, and comparison remain missing. |
| Subscriptions | PARTIAL | A tab-owned service/Cubit, bounded timeline, connect/disconnect controls, and lifecycle test now exist. Reconnect/resubscribe, connection-parameter secret resolution, session persistence/history/comparison, advanced timeline filters/export, and complete UI remain missing. |
